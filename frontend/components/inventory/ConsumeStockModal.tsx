...
import { postMovementOut } from "@/lib/store/inventory";
...
export function ConsumeStockModal({
  ...
  async function submit() {
    ...
    setLoading(true);
    try {
      await postMovementOut({ itemId, quantity: n, reason: reason || undefined });
      showToast("Consumption recorded", "info");
      onDone();
      onClose();
    } catch (e) {
      showToast(apiErrorMessage(e, "Failed to record consumption"), "error");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="na-modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="na-glass-card w-full max-w-md rounded-xl p-5">
        <h2 className="text-lg font-semibold text-[var(--na-text)]">Consume stock</h2>
        ...
        <label className="mt-3 block text-xs font-bold uppercase text-[var(--na-muted)]">
          ...
          <input
            ...
            placeholder="Procedure / department"
          />
        </label>
